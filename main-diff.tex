...
%DIF ADD main.tex             Sun Jul  5 17:24:45 2026
%% ---------------------------------
...
The simulations were \DIFdelbegin \DIFdel{validated }\DIFdelend \DIFaddbegin \DIFadd{evaluated }\DIFaddend using the CLIMATE-GRID (CG) dataset, a Belgian 5 km gridded observational dataset based on interpolated station data and developed at the Royal Meteorological Institute of Belgium. 
...
\DIFdelbegin \DIFdel{Rainfall }\DIFdelend \DIFaddbegin \DIFadd{Precipitation }\DIFaddend data from all networks were aggregated to an hourly resolution to align with the temporal resolution of the simulation output.
...
Therefore, this \DIFdelbegin \DIFdel{validation }\DIFdelend \DIFaddbegin \DIFadd{evaluation }\DIFaddend study also focused on the statistics of extreme precipitation over Belgium. 
...
Note that the \DIFdelbegin \DIFdel{rainfall }\DIFdelend \DIFaddbegin \DIFadd{precipitation }\DIFaddend data were aggregated and not averaged over a region.
...
All simulations overestimate \DIFdelbegin \DIFdel{rainfall }\DIFdelend \DIFaddbegin \DIFadd{precipitation }\DIFaddend for all hours. 
...
The observational return levels decrease linearly with \DIFdelbegin \DIFdel{rainfall }\DIFdelend \DIFaddbegin \DIFadd{precipitation }\DIFaddend duration on a log-log scale (see Fig.~\ref{fig:IDFs}).
...
Our \DIFdelbegin \DIFdel{ALARO-4m simulations reproduce }\DIFdelend \DIFaddbegin \DIFadd{ALARO-4km simulation reproduces }\DIFaddend this constant diurnal pattern better than those in \citet{de_troch_application_2016}.
...
This discrepancy is most pronounced in the ALARO-25km simulation and is minimized in the ALARO-4km simulation, indicating that the coarser resolution more significantly overestimates precipitation amounts due to an overly high \DIFdelbegin \DIFdel{rainfall }\DIFdelend \DIFaddbegin \DIFadd{precipitation }\DIFaddend frequency. 
These results imply that drizzle contributes substantially to the simulated \DIFdelbegin \DIFdel{rainfall}\DIFdelend \DIFaddbegin \DIFadd{precipitation}\DIFaddend , particularly in the lower-resolution model.
