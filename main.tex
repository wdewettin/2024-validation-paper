...
The observational return levels decrease linearly with precipitation duration on a log-log scale (see Fig.~\ref{fig:IDFs}).
...
\caption{Relative error of the return-level intensities for each region (rows: Low-, Middle-, High-Belgium) and return period (columns: 2, 10, 50 years), using all available years for each dataset (observational record lengths are given in Table~\ref{tab:stations}; simulations span 1992--2022). Panels (a)--(i) are ordered row-wise. Each box plot represents the uncertainty as gauged by the bootstrap procedure with $10^4 \times 10^4$ ($= 10^8$) samples. The whiskers span the width of the 95\%-confidence interval around the median.}
...
This discrepancy is most pronounced in the ALARO-25km simulation and is minimized in the ALARO-4km simulation, indicating that the coarser resolution more significantly overestimates precipitation amounts due to an overly high precipitation frequency. 
These results imply that drizzle contributes substantially to the simulated precipitation, particularly in the lower-resolution model.
